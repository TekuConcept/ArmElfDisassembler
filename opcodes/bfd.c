...
// #include "bfdlink.h"
// #include "libbfd.h"
#include "arm/coff/internal.h"
// #include "coff/sym.h"
#include "arm/libcoff.h"
// #include "libecoff.h"
// #undef obj_symbols
#include "arm/elf-bfd.h"
    ...
    union _bfd_doprnt_args *args)
{
    ...
    while (*ptr != '\0') {
        ...
        if (*ptr != '%') {
	        ...
	        else result = fprintf (stream, "%s", ptr);
	        ptr += result;
	    }
        else if (ptr[1] == '%') {
	        fputc ('%', stream);
            ...
            *sptr = '\0';
	        ...
	        case 'c': {
                ...
                if (short_width)
		            ...
		            switch (wide_width) {
		            case 0: PRINT_TYPE (int, i); break;
		            case 1: PRINT_TYPE (long, l); break;
		            case 2:
		            default:
	        ...
	        case 's': PRINT_TYPE (char *, p); break;
	        case 'p':
                if (*ptr == 'A') {
		            asection *sec;
		            bfd *abfd;
		            ...
		            else result = fprintf (stream, "%s", sec->name);
		        }
	            else if (*ptr == 'B') {
		            bfd *abfd;
		            ...
		            if (abfd == NULL)
				        ...
				        abfd->filename);
		            else result = fprintf (stream, "%s", abfd->filename);
		        }
	            else
		            PRINT_TYPE (void *, p);
	            break;
        ...
        if (*ptr != '%') {
	        ...
	        case 'c': {
		        if (short_width)
		            ...
		            switch (wide_width) {
		            case 0: arg_type = Int; break;
		            case 1: arg_type = Long; break;
		            case 2:
		            default:
                    #if defined (__GNUC__) || defined (HAVE_LONG_LONG)
			            arg_type = LongLong;
    ...
    va_list ap)
{
    ...
    for (i = 0; i < arg_count; i++) {
        switch (args[i].type) {
	    case Int: args[i].i = va_arg (ap, int); break;
	    case Long: args[i].l = va_arg (ap, long); break;
	    case LongLong: args[i].ll = va_arg (ap, long long); break;
	    case Double: args[i].d = va_arg (ap, double); break;
	    case LongDouble: args[i].ld = va_arg (ap, long double); break;
	    case Ptr: args[i].p = va_arg (ap, void *); break;
	    default: abort ();
	    }
    }
