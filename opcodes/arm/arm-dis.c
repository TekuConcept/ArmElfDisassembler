...
num_instructions_vpt_block(long given)
{
    ...
    if (mask == 0) return 0;
    if (mask == 8) return 1;
    if ((mask & 7) == 4) return 2;
    if ((mask & 3) == 2) return 3;
    if ((mask & 1) == 1) return 4;
    return 0;
}
...
{
    if (astate == PRED_THEN) return PRED_ELSE;
    else if (astate == PRED_ELSE) return PRED_THEN;
    else return PRED_NONE;
}
...
update_next_predicate_state(void)
{
    ...
    switch (vpt_block_state.current_insn_num) {
        case 1: mask_for_insn = 8; break;
        case 2: mask_for_insn = 4; break;
        case 3: mask_for_insn = 2; break;
        case 4: return PRED_NONE;
    }

    if (pred_mask & mask_for_insn)
         return invert_next_predicate_state (vpt_block_state.next_pred_state);
    else return vpt_block_state.next_pred_state;
}
	...
	bfd_boolean print_shift)
{
    ...
    if ((given & 0xff0) != 0) {
        ...
        else func (stream, ", %s", arm_regnames[(given & 0xf00) >> 8]);
    }
}
	...
	enum mve_instructions matched_insn)
{
    ...
    if (p == 1) {
        ...
        /* Pre-indexed mode.  */
        else func (stream, "[%s, #%s%lu]!", arm_regnames[gpr], add_sub, mod_imm);
    }
    else if ((p == 0) && (w == 1))
        /* Post-index mode.  */
        func (stream, "[%s], #%s%lu", arm_regnames[gpr], add_sub, mod_imm);
	...
	enum mve_undefined* undefined_code)
{
    *undefined_code = UNDEF_NONE;
    ...
    case MVE_VSRI: {
	    ...
	    if ((sz & 7) == 1) return FALSE;
	    else if ((sz & 6) == 2) return FALSE;
	    else if ((sz & 4) == 4) return FALSE;
	    else {
	        *undefined_code = UNDEF_SIZE;
	        return TRUE;
	    }
    ...
    case 32:
        if (isfloat) {
	        ...
	        floatformat_to_double
	            (&floatformat_ieee_single_little, valbytes, &fvalue);

	        func (stream, "#%.7g\t; 0x%.8lx", fvalue, value);
	    }
        else
	        func (stream, printU
                ? "#%lu\t; 0x%.8lx"
		        ...
		        && !printU ? value | ~0xffffffffL : value),
	            value);
        break;
    case 64:
	...
	enum mve_instructions matched_insn)
{
    ...
    case MVE_VST2:
        ...
        else func (stream, "<illegal reg q%ld>", q_reg_start);
        break;
    case MVE_VLD4:
    case MVE_VST4:
        if (q_reg_start <= 4)
	         func (stream, "{q%ld, q%ld, q%ld, q%ld}", q_reg_start,
	            q_reg_start + 1, q_reg_start + 2, q_reg_start + 3);
        else func (stream, "<illegal reg q%ld>", q_reg_start);
        break;
	...
	enum mve_instructions matched_insn)
{
    ...
    case MVE_VDUP:
        switch (size) {
	    case 0: func (stream, "32"); break;
	    case 1: func (stream, "16"); break;
	    case 2: func (stream, "8"); break;
	    default: break;
	    }
        break;
    ...
    case MVE_VMOV_VEC_LANE_TO_GP:
        switch (size) {
	    case 0: case 4: func (stream, "32"); break;
	    case 1: case 3:
	    case 5: case 7: func (stream, "16"); break;
	    case 8: case 9: case 10: case 11:
	    case 12: case 13: case 14: case 15: func (stream, "8"); break;
	    default: break;
    	}
        break;
    ...
    case MVE_VMOV_IMM_TO_VEC:
	    ...
	    case 12: case 24: case 26: func (stream, "i32"); break;
	    case 16: case 20: func (stream, "i16"); break;
	    case 28: func (stream, "i8"); break;
	    case 29: func (stream, "i64"); break;
        case 30: func (stream, "f32"); break;
	    default: break;
        }
        break;
    ...
    case MVE_VMVN_IMM:
	    ...
	    case 6: case 12: case 13: func (stream, "32"); break;
	    case 8: case 10: func (stream, "16"); break;
	    default: break;
	    }
        break;
    ...
    case MVE_VORR_IMM:
        switch (size) {
	    case 1: case 3:
	    case 5: case 7: func (stream, "32"); break;
        case 9: case 11: func (stream, "16"); break;
	    default: break;
	    }
        break;
    ...
    case MVE_VSHRN: {
	    switch (size) {
	    case 1: func (stream, "16"); break;
	    case 2: case 3: func (stream, "32"); break;
	    default: break;
	    }
    } break;
    ...
    case MVE_VSRI: {
	    switch (size) {
	    case 1: func (stream, "8"); break;
	    case 2: case 3: func (stream, "16"); break;
	    case 4: case 5: case 6: case 7: func (stream, "32"); break;
	    default: break;
	    }
    } break;
    default: break;
	...
	enum mve_instructions matched_insn)
{
    ...
    if (matched_insn == MVE_VSHLL_T1) imm6 &= 0x1f;
    ...
    else print_mve_undefined(info, UNDEF_SIZE_0);
...
static bfd_boolean
print_insn_coprocessor_1(
    const struct sopcode32 *opcodes,
	bfd_vma pc,
	struct disassemble_info *info,
	long given,
	bfd_boolean thumb)
        ...
        for (c = insn->assembler; *c; c++) {
            if (*c == '%') {
                ...
                case 'B': {
                    ...
                    else func (stream, "{d%d-d%d}", regno, regno + offset - 1);
                } break;
                case 'C': {
                ...
                case 'J': {
                    unsigned long regno
                        = arm_decode_field_multiple (given, 13, 15, 22, 22);
                    switch (regno) {
                    case 0x1: func (stream, "FPSCR"); break;
                    case 0x2: func (stream, "FPSCR_nzcvqc"); break;
                    case 0xc: func (stream, "VPR"); break;
                    case 0xd: func (stream, "P0"); break;
                    case 0xe: func (stream, "FPCXTNS"); break;
                    case 0xf: func (stream, "FPCXTS"); break;
                    default: func (stream, "<invalid reg %lu>", regno); break;
                    }
                } break;
                case 'F':
                    switch (given & 0x00408000) {
                    case 0: func (stream, "4"); break;
                    case 0x8000: func (stream, "1"); break;
                    case 0x00400000: func (stream, "2"); break;
                    default: func (stream, "3");
                    }
                    break;
                case 'P':
                    switch (given & 0x00080080) {
                    case 0: func (stream, "s"); break;
                    case 0x80: func (stream, "d"); break;
                    case 0x00080000: func (stream, "e"); break;
                    default: func (stream, _("<illegal precision>")); break;
                    }
                    break;
                case 'Q':
                    switch (given & 0x00408000) {
                    case 0: func (stream, "s"); break;
                    case 0x8000: func (stream, "d"); break;
                    case 0x00400000: func (stream, "e"); break;
                    default: func (stream, "p"); break;
                    }
                    break;
                case 'R':
                    switch (given & 0x60) {
                    case 0: break;
                    case 0x20: func (stream, "p"); break;
                    case 0x40: func (stream, "m"); break;
                    default: func (stream, "z"); break;
                    }
                    break;
                ...
                case '5': case '6': case '7': case '8': case '9': {
                    ...
                    case 'f':
                        ...
                        else func (stream, "f%ld", value);
                        break;
                    case 'w':
                        if (width == 2)
                                func (stream, "%s", iwmmxt_wwnames[value]);
                        else func (stream, "%s", iwmmxt_wwssnames[value]);
                        break;
                    case 'g':
                    ...
                    case 'c':
                        switch (value) {
                        case 0: func (stream, "eq"); break;
                        case 1: func (stream, "vs"); break;
                        case 2: func (stream, "ge"); break;
                        case 3: func (stream, "gt"); break;
                        default: func (stream, "??"); break;
                        }
                        break;
                    case '`':
                        c++;
	                ...
	                if (*c == '%') {
	                    ...
	                    case '5': case '6': case '7': case '8': case '9': {
		                    ...
		                    case 'n':
		                        ...
		                        else func (stream, "%s", arm_regnames[value]);
		                        break;
		                    case 'T':
	            ...
	            if (*c == '%') {
		            ...
		            case 'A': {
			            ...
			            if (stride > 1)
			                ...
			                func (stream, "d%d", rd);
			            else func (stream, "d%d-d%d", rd, rd + n - 1);
		            ...
		            case 'B': {
                        ...
                        case 4:
                            ...
                            else align = (idx_align & 1) ? 32 << size : 0;
                            break;
                        default: abort ();
                        }
		            ...
		            case 'C': {
                        ...
                        if (align) {
			                ...
			                else func (stream, " :%d", align);
			            }
			            func (stream, "]");
			            if (rm == 0xd) func (stream, "!");
			            else if (rm != 0xf) func (stream, ", %s", arm_regnames[rm]);
		            } break;
		            case 'D': {
		            ...
		            case 'F': {
			            ...
			            else func (stream, "{d%d-d%d}", regno, regno + num);
		            } break;
                    case '0': case '1': case '2': case '3': case '4':
                    case '5': case '6': case '7': case '8': case '9': {
			            int width;
        ...
        if (((given & insn->mask) == insn->value)
	        ...
	        for (c = insn->assembler; *c; c++) {
	            if (*c == '%') {
                    ...
                    case 'v': print_instruction_predicate (info); break;
                    case 'w':
                    ...
                    case '5': case '6': case '7': case '8': case '9': {
                        ...
                        case 'u':
                            switch (value) {
                            case 0: func (stream, "1"); break;
                            case 1: func (stream, "2"); break;
                            case 2: func (stream, "4"); break;
                            case 3: func (stream, "8"); break;
                            default: break;
                            }
                            break;
                        case 'o':
                        ...
                        case 'd':
                            if (insn->mve_op == MVE_VQSHL_T2
                                ...
                                switch (value) {
                                case 0x00: func (stream, "8"); break;
                                case 0x01: func (stream, "16"); break;
                                case 0x10:
    ...
    FOR_EACH_DISASSEMBLER_OPTION (opt, options) {
        if (CONST_STRNEQ (opt, "reg-names-")) {
	        ...
	        if (i >= NUM_ARM_OPTIONS)
	            /* xgettext: c-format */
	            opcodes_error_handler (_("unrecognised register name set: %s"), opt);
	    }
	        ...
	        if (endptr != procptr + 1 || coproc_number > 7) {
	            opcodes_error_handler (_("cde coprocessor not between 0-7: %s"), opt);
	            continue;
	        }
	        if (*endptr != '=') {
	            opcodes_error_handler (_("coproc must have an argument: %s"), opt);
	            continue;
	        }
	        endptr += 1;
	...
	arm_feature_set * features)
{
    ...
    case bfd_mach_arm_ep9312:   ARM_SET_FEATURES (ARM_FEATURE_LOW (ARM_AEXT_V4T, ARM_CEXT_MAVERICK | FPU_MAVERICK)); break;
    ...
    case bfd_mach_arm_unknown:
        ARM_SET_FEATURES (ARM_FEATURE (-1,
			-1 & ~(ARM_EXT2_MVE | ARM_EXT2_MVE_FP), -1));
        break;
    default: abort ();
    }
    #undef ARM_SET_FEATURES
    ...
    bfd_boolean little)
{
    ...
    if (info->symbols != NULL) {
        ...
        else if (bfd_asymbol_flavour (*info->symbols) == bfd_target_elf_flavour && !found) {
